...
public class ComponentAutoBindToolInspector : Editor
{
    ...
    private string[] s_AssemblyNames = { "HotUpdate" };
    ...
    private void WriteUsing(StreamWriter streamWriter) 
    {
        ...
        for (int i = 0; i < m_Target.BindDatas.Count; i++)
        {
            ...
            if ((data.BindCom.GetType().Name == "TextMeshProUGUI" || data.BindCom.GetType().Name == "TMP_Dropdown") && nameStr.IndexOf("using TMPro;") < 0)
            {
                streamWriter.WriteLine("using TMPro;");
                nameStr += "using TMPro;";
            }
            else if (data.BindCom.GetType().Name == "SkeletonGraphic" && nameStr.IndexOf("using Spine.Unity;") < 0 )
            {
                streamWriter.WriteLine("using Spine.Unity;");
                nameStr += "using Spine.Unity;";
            }
            else if (data.BindCom.GetType().Name == "UnityArmatureComponent" && nameStr.IndexOf("using DragonBones;") < 0)
            {
                streamWriter.WriteLine("using DragonBones;");
                nameStr += "using DragonBones;";
            }
