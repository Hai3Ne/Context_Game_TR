...
namespace EditorTool
{
    class AnimationOpt
    {
        ...
        void _OptmizeAnimationFloat_X(uint floatSize)
        {
            if (_clip != null && floatSize > 0)
            {
                //浮点数精度压缩到f3
                ...
                Keyframe[] keyFrames;
                string floatFormat;
                if (_FLOAT_FORMAT.TryGetValue(floatSize, out floatFormat))
                {
                ...
                {
                    Debug.LogErrorFormat("目前不支持{0}位浮点", floatSize);
                }
            }
        }
