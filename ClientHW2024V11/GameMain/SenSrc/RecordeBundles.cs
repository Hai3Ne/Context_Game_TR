...
public class RecordeBundles
{
    ...
    private Queue<KeyValuePair<int, string>> _subBundles = new Queue<KeyValuePair<int, string>>();
    ...
    {
        //#if UNITY_EDITOR
        _recordeBundle = false;// ClientSetting.Instance.GetBoolValue("RecordeBundle");
        if (_recordeBundle)
        {
            

            string recordDir = GameConst.DataPath + "FirstResTool/Config";
            ...
            Recorde(AppConst.UIBundName + "/ui/prefabs/versionupdate.unity3d");
...
//#if UNITY_EDITOR
            lock (_lockObj)
            {
                string file = path.Replace(GameConst.DataPath, "").Replace(Util.AppContentPath(), "");
                LogMgr.Log("RecordeBundle:" + file);
                if (!_bundles.Contains(file))
                {
                    _bundles.Add(file);
                   /* var lv = PlayerData.isInit ? 1000 : 0;
                    _subBundles.Enqueue(new KeyValuePair<int, string>(lv, file));*/
                }
            }
//#endif
        }
        ...
        while (true)
        {
            lock (_lockObj)
            {
                if (_subBundles.Count > 0)
                {
                    ...
                    {
                        kv = _subBundles.Dequeue();
                        sw.WriteLine(string.Format("{0}minute:Lv:{1}:{2}", Mathf.FloorToInt((float)minu) + 1, kv.Key, kv.Value));
                    }
