...
public class DecalPolygon {
	...
	public static DecalPolygon ClipPolygon (DecalPolygon polygon, Plane plane) {
        ...
        if (positiveCount == 0) return null; // полностью за плоскостью  google翻译(俄语)：完全落于平面后  by lucifezhang
        if (positiveCount == polygon.vertices.Count) return polygon; // полностью перед плоскостью  google翻译(俄语)：完全落于平面前 by lucifezhang
