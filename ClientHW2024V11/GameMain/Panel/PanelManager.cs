...
public class PanelManager :MonoBehaviour
{
    ...
    protected Dictionary<string, MainPanelUICfg> mPrefabMap = new Dictionary<string, MainPanelUICfg>();   //所有UI的配置
    ...
    public int layerHide = 0; //LayerMask.NameToLayer("outui");
    ...
    {
        layerHide = LayerMask.NameToLayer("outui");
    }
    ...
    public virtual void Init()
    {
        ...
        if (uUIRootObj == null)
        {
            ...
            PopUI = uUIRootObj.transform.Find("PopUI");
    ...
    public void LoadPanel(MainPanelUICfg mpUI, string panelName)
    {
        ...
        switch (mpUI.enumLayerType)
        {
            case 0:
                panelObj.transform.SetParent(uUIRootObj.transform.Find("BackgroundUI"));
                break;
            case 1:
                panelObj.transform.SetParent(uUIRootObj.transform.Find("PopUI"));
                break;
            case 2:
                panelObj.transform.SetParent(uUIRootObj.transform.Find("TopUI"));
                break;
            default:
                break;
        }
    ...
    public void ShowPanel(string panelName,object param = null, bool hasEffect = true, System.Action finishCallBack = null)
    { 
        if (uiCamera == null)
      ...
      StartCoroutine(delayHide(panelName, tempPanelName, param,finishCallBack)); 
    }

    //
    IEnumerator delayHide(string showName, string hideName,object param = null, System.Action finishCallBack = null)
    {
...
//             waitDialog.Show(true);
        ...
        if (hidePanel != null)
        {
            MainPanelMgr.Instance.HidePanel(hideName);
            //hidePanel.Hide();
        }
            
        if (newPanel != null)
        {
            newPanel.param = param;
            newPanel.Show();
    ...
    public string CurPanelName
    {
        ...
        { 
            ...
            mCurPanelName = value;
            if (!string.IsNullOrEmpty(mCurPanelName) && !m_PanelNameStack.Contains(mCurPanelName))
            {
    ...
    {  
        ...
        {
            if (mShowDlgList[i] != null && mShowDlgList[i].Layer != 3)
            {
    ...
    public void CheckMaxCache()
    {
        ...
        while (isNext)
        {
            ...
            {
                ++hidenumber;
                if (mintime == -1 || (mintime > panel.LastShowTime &&  panel.LastShowTime > 0))
                {
        ...
        //已有缓存，不做删除
        if (lastpanel != null && CurPanelName == lastpanel.name)
        {
            lastpanel = null;
        }
        e.Dispose();
    ...
    public void HideAllPanel(bool disable = false)
    {
    ...
    public PanelBase ShowDialog(string panelName, bool hasEffect = true, object param  = null,int depth = 0)
    {
        if (!HasPanel(panelName))
        {
            LogMgr.UnityLog("has not pannel:" + panelName);
    ...
    public bool bHideAllPopPanel()
    {
        for(int i = 0;i < PopUI.childCount;i++)
        {
            if (PopUI.GetChild(i).gameObject.activeSelf)
                return false;
        }
        return true;
    }
