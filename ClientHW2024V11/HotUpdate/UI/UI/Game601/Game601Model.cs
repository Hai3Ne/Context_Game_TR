...
namespace HotUpdate
{
    public class Game601Model : Singleton<Game601Model>
    {
        ...
        public List<SGame6AwardInfo> arrayAward = new List<SGame6AwardInfo>();

        public string[] AniNames = new string[12] { "", "k", "a", "saozi", "qizi", "laba", "shoutao", "yifu", "jxz", "jkp", "scatter", "wild" };

        public TO_Spin_601 toSpin { get; private set; } = new TO_Spin_601(); //免费数据
        ...
        public void Init(SC_GAME6_ROOM_INFO roomInfo)
        {
            lastCount = 0;
        ...
        {
            JackPotList.Clear();
            Dictionary<int, Game601_JackPot> Configs = ConfigCtrl.Instance.Tables.TbGame601_JackPot.DataMap;
            for (int i = 1; i <= Configs.Count; i++)
            {
                Game601_JackPot config = Configs[i];
            ...
            var config0 = ConfigCtrl.Instance.Tables.TbGame600_Gear.Get(nRoomType);
            GearList.Clear();
            ...
            for (int i = 1; i <= 9; i++)
            {
                ...
                var config = ConfigCtrl.Instance.Tables.TbGame600_Line_Config.Get(i);
        ...
        public void SetSpinData(SC_GAME6_BET_RET msg)
        {
            bShowFreeAni = false;
            slotResult.Clear();
            ...
            Game601Model.Instance.n64Jackpot = msg.n64Jackpot;
