...
namespace HotUpdate
{
    public class Room602Mgr : Singleton<Room602Mgr>
    {
        public UIRoom602_SlotCpt slot;
        public TO_Spin_602 toSpin;
        ...
        public void Init(UIRoom602_SlotCpt slot, TO_Spin_602 spin)
        {
            this.slot = slot;
            this.toSpin = spin;
            StepList.Clear();
        ...
        {
            if (toSpin.WinGold > 0 || Game602Model.Instance.lines.Count > 0)
            {
                slot.showLines();
                if (slot.isFreeSpin)
        ...
        public void StepFreeEnd()
        {
            ...
            if (Game602Model.Instance.bShowFreeAni)
                delayTimes = 2;
            ...
            {
                if (toSpin.rate > 2 || Game602Model.Instance.toSpin.n64FreeGold > 0 || toSpin.n64RSPowerGold > 0)
                    return;
                CoreEntry.gTimeMgr.AddTimer(delayTimes, false, () =>
                {
                    slot.continueSpin();
