...
namespace HotUpdate
{
    public partial class CommonTop : MonoBehaviour
    {
        ...
        public void SetDanJi(bool bDanJi,bool bShowTogAuto,bool bHasBroadCast = true,bool bMidBroadCast = false, int lineCount=1)
        {
            ...
            m_Trans_Gold.transform.gameObject.SetActive(bDanJi);
        ...
        {
            StartCoroutine(ToolUtil.GetHeadImage(Encoding.UTF8.GetString(MainUIModel.Instance.palyerData.m_szHeadUrl).Replace("\0", null), m_Btn_head.GetComponent<Image>()));
        ...
        private void RefreshVip()
        {
            ...
            //var currnetVipExp = ConfigCtrl.Instance.Tables.TbVip_lvl_Config[MainUIModel.Instance.palyerData.m_i4Viplev ].NeedExp / 100;
        ...
        {
            m_Txt_golds.text = ToolUtil.AbbreviateNumber(gold);// ToolUtil.ShowF2Num(gold);
        ...
        {
            m_Txt_golds.text = ToolUtil.AbbreviateNumber(MainUIModel.Instance.palyerData.m_i8Golds); //ToolUtil.ShowF2Num(MainUIModel.Instance.palyerData.m_i8Golds);
        }
        ...
        public void OnClickBtnMin()
        {
            ...
            {
                Message.Broadcast(MessageName.GE_COMMONTOP_MINSBTN);
        ...
        private void OnClickBtnAdd()
        {
            if (MainUIModel.Instance.RoomData.nGameType == 4)
            {
                Message.Broadcast(MessageName.GE_COMMONTOP_ADDBTN);
            }
            else
            {
                if (ClickCondition != null)
        ...
        private void OnClickBtnMax()
        {
            if (MainUIModel.Instance.RoomData.nGameType == 4)
            {
                Message.Broadcast(MessageName.GE_COMMONTOP_MAXBTN);
            }
            else
            {
                if (ClickCondition != null)
        ...
        public void onPointerUp()
        {
            ...
            if(MainUIModel.Instance.RoomData.nGameType == 4)
            {
                Message.Broadcast(MessageName.GE_COMMONTOP_ROLLBTN);
        ...
        {
            m_Txt_ID.text = name;
            m_TxtM_Money.text = ToolUtil.ShowF2Num2(gold);// ( / 10000).ToString("f2");
            string imgurl = "" + nIconID;
       ;
