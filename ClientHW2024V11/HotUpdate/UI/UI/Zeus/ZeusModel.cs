...
namespace HotUpdate
{
    public class ZeusModel : Singleton<ZeusModel>
    {
        ...
        /// 玩家中奖列表
        /// </summary>
        public List<SGame4AwardInfo> awardList = new List<SGame4AwardInfo>();

        
        public List<ZeusRewardData> gameData = new List<ZeusRewardData>();

        public Dictionary<int, List<ZeusMultipleInfo>> ZeusMultipleDic = new Dictionary<int, List<ZeusMultipleInfo>>();
        /// <summary>
        /// 设置游戏房间信息
        /// </summary>
        /// <param name="data"></param>
        public void setRoomData(SC_GAME4_ROOM_INFO data)
        {
        ...
        public void InitZeusConfig()
        {
            var list = ConfigCtrl.Instance.Tables.TbZeus_Config.DataList;
            ZeusMultipleDic.Clear();
            for (int i = 0; i < list.Count; i++)
            {
                var list1 = new List<ZeusMultipleInfo>();
                var data = list[i];
                ...
                for (int j = 0; j < arr.Length; j++)
                {
                    ...
                    ZeusMultipleInfo info = new ZeusMultipleInfo();
                ...
                ZeusMultipleDic[data.Id] = list1;
            }
        }

        public ZeusMultipleInfo FindConfigByInterval(int id,int count)
        {
            ZeusMultipleInfo info = null;
            if (ZeusMultipleDic.ContainsKey(id))
            {
                for (int i = ZeusMultipleDic[id].Count - 1; i >= 0 ; i--)
                {
                    if(ZeusMultipleDic[id][i].min <= count)
                    {
                        info = ZeusMultipleDic[id][i];
                        break;
                    }
                }
            }
        ...
        /// 下注返回中奖信息
        /// </summary>
        public void setGameInfo(SC_GAME4_BET_RET data)
        {
            Gold = data.n64Gold;
            ...
            {
                ZeusRewardData newData = new ZeusRewardData();
                ...
                newData.list = new List<ZeusRewardInfo>();
                for (int j = 0; j < data.sInfo[i].arrayLogo.Length; j++)
                {
                    ZeusRewardInfo info = new ZeusRewardInfo();
                    info.id = (int)data.sInfo[i].arrayLogo[j];
                    var config = ConfigCtrl.Instance.Tables.TbZeus_Config.Get(info.id);
                    if(config.Type == 1)
                    {
        ...
        public void AddRoomInfoList(SGame4AwardInfo[] info)
        {
        ...
        public void SetInitData()
        {
            ...
            ZeusRewardData data = new ZeusRewardData();
            data.n64CommPowerGold = 0;
            data.n64RSPowerGold = 0;
            data.n64TotalGold = 0;
            data.nTotalDouble = 0;
            var list = new List<ZeusRewardInfo>();
        ...
        public void getServerData(long gold,int bet)
        {
            ...
            while (isNext)
            {
                ...
                {
                    var config = ConfigCtrl.Instance.Tables.TbZeus_Config.Get(info.Key);
                    if (config.Beishu <= info.Value)
                    {
                        isNext = true;
                        var data = FindConfigByInterval(info.Key, info.Value);

                        num += data.multiple* data.times* bet ;
                    }
                }
