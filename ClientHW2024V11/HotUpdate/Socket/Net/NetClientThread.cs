...
/*            if ((--m_currentTry.tryCount) > 0)
        ...
        protected void dispatchMsg()
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                //消息派发
                if (m.cmd == (short)NetMsgDef.S_HEART_BEAT)
                {
                    //心跳
                    NetReadBuffer buf = NetReadBuffer.CacheBuff;
                    buf.Init(m.data, 0, m.length);
                    NetMgr.CacheRecvHeartBeat.unpack(buf);
                    NetMgr.netMgr.OnReceiveHearBeat(m.tick, NetMgr.CacheRecvHeartBeat);
                }
                else if (m.cmd == (short)NetMsgDef.S_FRAME_MSG)
                {
                    //合并包
        ...
        private void SendMessage(RecvBuffer msg)
        {
            ...
            MsgData msgdata = PacketBuilder.newBuilder(msg.cmd, buffer);
            if (msgdata != null)
            {
                NetHandler.Instance.OnPakckage(msg.cmd, msgdata);
            }
        ...
        public static void OnLuaDispatchMsg(short cmd, NetReadBuffer buffer)
        {
