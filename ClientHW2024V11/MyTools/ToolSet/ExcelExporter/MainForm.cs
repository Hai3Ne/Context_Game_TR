...
namespace ExcelExporter
{
    ...
    public partial class MainForm : Form
    {
        ...
        public void RefreshExportTree()
        {
            ...
            Dictionary<string, ExportInfo> exportinfos = ConfigArchive.Instance.ExportInfos;
            tv.Nodes.Clear();
        ...
        {
            if (!Directory.Exists(ConfigArchive.Instance.ExportFolderClientLua))
            {
                Directory.CreateDirectory(ConfigArchive.Instance.ExportFolderClientLua);
                MainForm.CurForm.Log("ExportFolderClientLua 导出路径不存在，创建 " + ConfigArchive.Instance.ExportFolderClientLua);
                //return;
            }
            if (!Directory.Exists(ConfigArchive.Instance.ExportFolderServerCPP))
            {
                Directory.CreateDirectory(ConfigArchive.Instance.ExportFolderServerCPP);
                MainForm.CurForm.Log("ExportFolderServerCPP 导出路径不存在，创建 " + ConfigArchive.Instance.ExportFolderServerCPP);
                //return;
            }
            if (!Directory.Exists(ConfigArchive.Instance.ExportFolderServerLua))
            {
                Directory.CreateDirectory(ConfigArchive.Instance.ExportFolderServerLua);
                MainForm.CurForm.Log("ExportFolderServerLua 导出路径不存在，创建 " + ConfigArchive.Instance.ExportFolderServerLua);
                //return;
            }
            if (!Directory.Exists(ConfigArchive.Instance.ExportFolderClientYYLua))
            {
                Directory.CreateDirectory(ConfigArchive.Instance.ExportFolderClientYYLua);
            }
        ...
        /// 生成CPP按钮。
        /// </summary>
        private void btnBuildCPP_Click(object sender, EventArgs e)
        {
            List<TreeNode> nodes = ttvDataList.CheckedLeafNodes;
            List<string> paths = new List<string>();
            foreach (TreeNode node in nodes)
            ...
            {
                OnExportStart();
                ConfigArchive.Instance.StartBuildCPP(paths, OnExportEnd);
            }
            else
            {
                Log("请选择要生成的数据");
