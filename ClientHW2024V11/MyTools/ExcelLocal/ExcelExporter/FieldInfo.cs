...
namespace ExcelExporter
{
    ...
    public class FieldInfo
    {
        ...
        {
            if (string.IsNullOrEmpty(vname) || string.IsNullOrEmpty(type) || string.IsNullOrEmpty(type) || string.IsNullOrEmpty(target))
            {
                return null;
            }
            //变量名 类型 目标都得有
            ...
            if (target.IndexOf('c') < 0 && target.IndexOf('s') < 0)
            {
                return null;
            }
            ...
            info.isLang = target.Contains("l") || target.Contains("L");
            return info;
        }
        ...
        /// 数据导出者。
        /// </summary>
        public DataExporter m_deExporter;
