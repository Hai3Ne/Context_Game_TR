...
public class UILabel : UIWidget
{
	...
	bool keepCrisp
	{
		get
		{
			...
			{
#if UNITY_IOS || UNITY_ANDROID || UNITY_WP8 || UNITY_WP_8_1 || UNITY_BLACKBERRY
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
        ...
        if (effectStyle != Effect.None)
		{
            ...
            if ((effectStyle == Effect.Outline) || (effectStyle == Effect.Outline8) || (effectStyle == Effect.Outline8AndShadow))
			{
				offset = end;
				end = verts.size;
                ...
                if (effectStyle == Effect.Outline8 || effectStyle == Effect.Outline8AndShadow)
				{
					offset = end;
					end = verts.size;
                    ...
                    if (effectStyle == Effect.Outline8AndShadow) {
                        offset = end;
                        end = verts.size;
                        ApplyShadow(verts, uvs, cols, offset, end, pos.x*2, -pos.y*2);
                    }
				}
			}
		}
	...
	public void ApplyShadow (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, int start, int end, float x, float y)
	{
        ...
        if (this.IsGray && mFont == null) {
            float g = (c.r * 0.30f + c.g * 0.59f + c.b * 0.11f) * 0.5f;// *0.5f + 0.25f;
            //c = this.mGrayColor * g;
            //c.a = mEffectColor.a;
            c = new Color(g, g, g, 1);
        }
	...
	public void UpdateNGUIText ()
	{
		...
		NGUIText.gradient = mApplyGradient && (mFont == null || !mFont.packedFontShader);// && this.IsGray == false;
        if (this.IsGray) {
            float g = (mGradientTop.r * 0.30f + mGradientTop.g * 0.59f + mGradientTop.b * 0.11f);// *0.5f + 0.25f;
            NGUIText.gradientTop = new Color(g, g,g, 1);

            g = (mGradientBottom.r * 0.30f + mGradientBottom.g * 0.59f + mGradientBottom.b * 0.11f);// *0.5f + 0.25f;
            NGUIText.gradientBottom = new Color( g, g, g, 1);
        } else {
