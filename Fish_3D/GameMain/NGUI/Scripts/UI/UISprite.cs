...
//			material.shader = mShader;
	...
	{
		Debug.Log($"[AtlasDebug] GameObject: {gameObject.name}");
		Debug.Log($"[AtlasDebug] mAtlas null? {mAtlas == null}");
    
		if (mAtlas != null)
		{
			Debug.Log($"[AtlasDebug] Atlas name: {mAtlas.name}");
			Debug.Log($"[AtlasDebug] Material null? {mAtlas.spriteMaterial == null}");
			Debug.Log($"[AtlasDebug] Replacement atlas: {(mAtlas.replacement != null ? mAtlas.replacement.name : "None")}");
			Debug.Log($"[AtlasDebug] Sprite count: {mAtlas.spriteList.Count}");
		}
    
		Debug.Log($"[AtlasDebug] SpriteName: {mSpriteName}");
		Debug.Log($"[AtlasDebug] Current sprite null? {mSprite == null}");
		Debug.Log($"[AtlasDebug] Material from property: {(material != null ? material.name : "NULL")}");
	}
}
