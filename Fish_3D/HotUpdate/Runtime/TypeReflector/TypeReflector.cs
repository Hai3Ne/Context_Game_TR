...
{
    public static object CreateObj(TypeData td, BinaryReader bin, int start_index = 0)//start_index:跳过属性数量
    {
        Type t = td.ClassType;

        int td_size = td.Size;
        for (int i = 0; i < start_index; ++i) {
            td_size -= td.Fields[i].OrgSize;
        }


        //int size = (int)(bin.BaseStream.Length - bin.BaseStream.Position);
        //if (td.parseFlag == 0 && size < td_size / td.ArrayCount)
        //{
        //    Debug.Log("字节流的长度小于类的长度, Bytes Size:" + bin.BaseStream.Length.ToString() +", Class["+td.ClassType+"] Size:" + (td.Size / td.ArrayCount).ToString());
        //    return null;
        //}

		if (td.parseFlag == 1) {			
			return NetRespCustomParse.Parse (t, bin);
		}
        ...
        for (int i = start_index; i < td.Fields.Length; ++i)
        {
            ...
            {
				if (curData.VarType || curData.ArrayCount == 0xFFFF)
                {
                    //变长 
                    int curSize = (int)(bin.BaseStream.Length - bin.BaseStream.Position);
                    if (curData.VarType && (curSize % curData.OrgSize != 0))
                    {
						Debug.Log("字节长度不是变长类型的整数倍, Bytes Size:" + curSize.ToString() + ", Class["+curData.ClassType+"] Size:" + td.OrgSize.ToString());
                        return null;
                    }

					if (curData.ArrayCount == 0xFFFF && !curData.VarType) 
					{
						curSize = (int)bin.ReadUInt16 ();
					} else {
            ...
            else if (curData.HashCode == TypeMapping.STRING_HASH)
            {
                ...
                if (curData.VarType)
                {
                    ...
                    if(curSize % 2 != 0)
                    {
                        Debug.Log("字符串长度不是字符的整数倍:"+ curSize.ToString());
                        return null;
                    }
                    if(curSize > 0)
            ...
            {
                Debug.Log("未知的类型:" + curData.Handle.FieldType.ToString());
                return null;
            }
        }
        return obj;
    ...
    public static object BytesToObj<T>(byte[] data, int offset, int length)
    {
        ...
        if (td == null)
        {
            Debug.Log("未注册的类型:" + TypeSize<T>.RUN_TYPE.ToString());
            return null;
        }
		...
		object netpack = null;
		try{
			netpack = CreateObj(td, bin);
		}catch(Exception e){
			Debug.LogError ("Parse NetStream Error:["+e.Message+"]. length:["+length+"]"+td.ClassType.ToString()+" "+JsonUtility.ToJson(netpack));
		}
		return netpack;
    }

    public static object BytesToObj(int hash, byte[] data, int offset, int length, int start_index)
    {
        TypeData td = TypeMapping.FindType(hash);
        if (td == null)
        {
            Debug.Log("未注册的类型:" + hash.ToString());
            return null;
        }
        System.IO.MemoryStream ms = new MemoryStream(data, offset, length);
        BinaryReader bin = new BinaryReader(ms);
		object netpack = null;
		try{
			netpack = CreateObj(td, bin, start_index);
		}catch(Exception ee){
			Debug.LogError ("Parse NetStream Error:["+ee.Message+"]. length:["+length+"]"+td.ClassType.ToString());
		}
		return netpack;
    }
	...
	public static int PrimitiveSizeof<T>()
	{
		int typesz = 0;
		int hash = TypeSize<T>.HASH;
		TypeData td = TypeMapping.FindType(hash);
        ...
        for(int i = 0; i < td.Fields.Length; ++i)
        {
            ...
            else if (curData.Handle.FieldType.IsArray)
            {
                if (curData.ClassType.IsPrimitive)
                {
                    ...
                    {
						if (curData.ArrayCount == 0xFFFF) {
							IList ll = f as IList;
							if (ll != null) {
								bin.Write ((ushort)ll.Count);
								if (ll.Count>0)
									iReader.Write (bin, f, ll.Count);
							}
						} else {
