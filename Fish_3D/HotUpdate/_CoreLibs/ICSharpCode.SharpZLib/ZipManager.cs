...
public class ZipManager {
    public static byte[] CompressLZMA(byte[] data) {//LZMA压缩
    ...
    public static byte[] DecompressLZMA(byte[] data) {//解压缩
    ...
    /// 解压Zip包
    /// </summary>
    public static byte[] UnzipFile(byte[] data) {//解压
        Stream input = new MemoryStream(data);
        ZipEntry entry = null;
        byte[] bytes = null;
        using (ZipInputStream zipInputStream = new ZipInputStream(input)) {
            //if (!string.IsNullOrEmpty(_password))
            //    zipInputStream.Password = _password;
            if (null != (entry = zipInputStream.GetNextEntry())) {
                // 写入文件
                bytes = new byte[zipInputStream.Length];
	...
	public static byte[] ZipFile(byte[] data, int start, int end) {
		byte[] buff = new byte[end - start];
		Array.Copy (data, start, buff, 0, buff.Length);
		buff = ZipFile (buff);
		byte[] newData = new byte[start + buff.Length];
		Array.Copy (data, 0, newData, 0, start);
		Array.Copy (buff, 0, newData, start, buff.Length);
		return newData;
	}
    /// <summary>
    /// 压缩文件
    /// </summary>
    public static byte[] ZipFile(byte[] data) {
        Stream input = new MemoryStream(data);
        ZipEntry entry = null;

        Stream output = new MemoryStream();
        using (ZipOutputStream zipOutputStream = new ZipOutputStream(output)) {
    ...
    /// 压缩文件
    /// </summary>
    public static void ZipFile(string file_path,byte[] data) {
        Stream input = new MemoryStream(data);
        ZipEntry entry = null;
        ...
        Stream output = File.Create(file_path);
        using (ZipOutputStream zipOutputStream = new ZipOutputStream(output)) {
            zipOutputStream.SetLevel(6);    // 压缩质量和压缩速度的平衡点
            //if (!string.IsNullOrEmpty(_password))
            //    zipOutputStream.Password = _password;
    ...
    private static bool ZipFile(string _filePathName, string _parentRelPath, ZipOutputStream _zipOutputStream, ZipCallback _zipCallback = null) {
        ...
        try {
            ...
            byte[] buffer = File.ReadAllBytes(_filePathName);
