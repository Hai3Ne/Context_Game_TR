...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public void DrawInspectorProperties (SerializedObject so, UIWidget w, bool drawColor)
	{
		...
		if (NGUIEditorTools.DrawHeader("Widget")) {
            ...
            NGUIEditorTools.DrawProperty("置灰", so, "mIsGray", GUILayout.MinWidth(20f));
            
            //bool is_gray = EditorGUILayout.Toggle("置灰（不保存）", w.IsGray);
            //if (is_gray != w.IsGray) {
            //    w.IsGray = is_gray;
            //    w.Invalidate(true);
            ...
            //}
            //if (w.IsGray && w is UILabel) {
            //    UILabel lb = w as UILabel;
            //    NGUIEditorTools.DrawProperty("Gray Tint", so, "mGrayColor", GUILayout.MinWidth(20f));
            //    //lb.mGrayColor = EditorGUILayout.ColorField("灰色", lb.mGrayColor);
            //}
