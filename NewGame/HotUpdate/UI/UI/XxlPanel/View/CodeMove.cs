    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Began)
            {
                ...
                Vector2 spritePoint = spriteRenderer.bounds.min + spriteRenderer.bounds.size / 2;

                if (Vector2.Distance(worldPoint, spritePoint) < spriteRenderer.bounds.size.x / 2)
                {
                    // 用户点击了Sprite
                    Debug.Log("Sprite touched!");
                    isTouch = true;
                    if (!isMoveing)
                    {
