...
namespace HotUpdate
{
    public class Room601Mgr : Singleton<Room601Mgr>
    {
        public UIRoom601_SlotCpt slot;
        public TO_Spin_601 toSpin;
        ...
        public void Init(UIRoom601_SlotCpt slot, TO_Spin_601 spin)
        {
            this.slot = slot;
            this.toSpin = spin;
            StepList.Clear();
        ...
        {
            if (toSpin.WinGold > 0 || Game601Model.Instance.lines.Count > 0)
            {
                slot.showLines();
                if (slot.isFreeSpin)
        ...
        public void StepFreeEnd()
        {
            ...
            if (Game601Model.Instance.bShowFreeAni)
                delayTimes = 2;
            ...
            {
                if (toSpin.rate > 2 || Game601Model.Instance.toSpin.n64FreeGold > 0 || toSpin.n64RSPowerGold > 0)
                    return;
                CoreEntry.gTimeMgr.AddTimer(delayTimes, false, () =>
                {
                    slot.continueSpin();
