...
public class PacketUtil
{
    ...
    public static byte[] compress_lz4(byte[] source ,int offset, int length ,out int compressLength)
    {
        try
        {
            int maxDestSize = LZ4psG.LZ4Codec.MaximumOutputLength(length);
            byte[] dest = new byte[maxDestSize];
             
            compressLength = LZ4psG.LZ4Codec.Encode32(source,offset,length,dest, 0,maxDestSize); 
        ...
        {
            SEZSJ.LogMgr.LogError(e.ToString());
        }
    ...
    public static int uncompress_lz4(byte[] source ,int offset, int length , byte[] dest, int destLength)
    {
        ...
        try
        {
            len = LZ4psG.LZ4Codec.Decode32(source, offset, length, dest, 0, destLength, false);           
        }
        catch(Exception e)
        {
            SEZSJ.LogMgr.LogError(e.ToString());
