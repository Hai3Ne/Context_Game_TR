    ...
    public class NetMgr : MonoBehaviour
    {
        ...
        public static NetMgr netMgr
        {
            get
            {
                ...
                {
                    m_netMgr = CoreEntry.CoreRootObj.AddComponent<NetMgr>();
                    m_netMgr.enabled = true;
                }
                return m_netMgr;
            }
        }
        ...
        public static float HEAR_BEAT_GAP = 6;                          //心跳发送间隔
        public static float HEAR_BEAT_TIME_OUT = 3;                    //心跳超时
        public int HEAR_BEAT_TIME_OUT_MAX = 2;                //最大超时次数
        ...
        void Start()
        {
            ...
            Message.Broadcast(MessageName.NET_INIT);
            // 各系统注册网路消息
            //MainRole.Instance.RegisterNetMsg();
/*            int nCount = ClientSetting.Instance.GetIntValue("HEAR_BEAT_TIME_OUT_MAX");
            if(nCount > 0)
            {
                Debug.Log("心跳超时最大次数来自配置 HEAR_BEAT_TIME_OUT_MAX ");
                HEAR_BEAT_TIME_OUT_MAX = nCount;
            }*/
            CoreEntry.gEventMgr.AddListener(SEZSJ.GameEvent.GE_THIRDPARTY_LOGOUT, ConfirmLogout);
            CoreEntry.gEventMgr.AddListener(SEZSJ.GameEvent.GE_EndPause, EndPause);
        }
        ...
        private void EndPause(GameEvent ge, EventParameter parameter)
        {
            if (netMgr != null && netMgr.Reconnect != null)
            {
                ...
                NetMgr.netMgr.Reconnect.needShowNotice = false;
        ...
        private void ConfirmLogout(GameEvent ge, EventParameter parameter)
        {
            Reconnect.onBtnGotoLogin();
        }
       ...
       static public void sendHeartbeat()
        {
            //UnityEngine.Debug.Log("Send HeartBeat:: 9999999999999999999999");
            //MsgData_cHeartBeat CacheHeartBeat = new MsgData_cHeartBeat();
            //CacheHeartBeat.Time = UiUtil.GetNowTimeStamp();
            //NetMgr.netMgr.send((Int16)NetMsgDef.C_HEART_BEAT, CacheHeartBeat);
        }
        ...
        void Update()
        {
            ...
            if (m_net != null)
            {
                ...
                if (m_page == 2 )
                {
                    if (m_net.m_tcpClient == null || !m_net.m_tcpClient.Connected)
                    {
                        return;
                    }
                    if (m_HearBeatState == 0)
                    {
                        ...
                        if (m_HearBeatGapCount <= 0)
                        {
                            ...
                            m_HearBeatLastTime = ToolUtil.getServerTime();
                            if(m_HearBeatTimeOutCount > 1)
                            {
                                Debug.Log("心跳包 ：：： 发送 超时次数：" + m_HearBeatTimeOutCount.ToString());
                                Debug.Log("  超时时间点：" + m_HearBeatSendTimeStamp);
                            }
                    ...
                    else if (m_HearBeatState == 1)
                    {
                        if (Time.realtimeSinceStartup - m_HearBeatSendTimeStamp > HEAR_BEAT_TIME_OUT)
                        {
                            ...
                            if (m_net.m_tcpClient == null || !m_net.m_tcpClient.Connected)
                            {
                                m_HearBeatTimeOutCount = 100;
                            }
                            if (m_HearBeatTimeOutCount > HEAR_BEAT_TIME_OUT_MAX)
                            {                                
                                LogMgr.Log("心跳包 超时次数:" +  m_HearBeatTimeOutCount);
                                m_HearBeatTimeOutCount = 0;
                                Debug.Log("断开时间戳：" + Time.realtimeSinceStartup);                              
        ...
        public void send(NetMsgDef cmd, MsgData data = null)
        {
            if (m_net != null)
            {
...
/*        public void send(Int16 cmd, LuaTable table)
        ...
        public void bindMsgHandler(NetMsgDef cmd, MsgHandler handler)
        {
            if (m_net != null)
            {
        ...
        public void SaveNetFlow(int sent, int recv)
        {
            ...
            string msg = string.Format("{2} down:{0}B/s      up:{1}B/s", CommonTools.NumberToString((int)recvflow, 6), CommonTools.NumberToString((int)sentflow, 6), t);
            LogMgr.Log(msg);
