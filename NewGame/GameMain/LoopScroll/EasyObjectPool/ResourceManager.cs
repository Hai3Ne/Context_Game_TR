...
public class ResourceManager : MonoBehaviour
{
    ...
    public static ResourceManager Instance
    {
        get
        {
            if (mInstance == null)
            {
                ...
                {
                    LogMgr.LogWarning("[ResourceManager] You'd better ignore ResourceManager in Editor mode");
                }
            }
    ...
    public void InitPool(string poolName, int size, PoolInflationType type = PoolInflationType.DOUBLE)
    {
        ...
        {
            GameObject pb = SEZSJ.CoreEntry.gResLoader.Load(poolName) as GameObject;
            if (pb == null)
            {
    ...
    public GameObject GetObjectFromPool(string poolName, bool autoActive = true, int autoCreate = 0)
    {
        ...
        if (poolDict.ContainsKey(poolName))
        {
            ...
            if (result == null)
            {
                LogMgr.LogWarning("[ResourceManager]:No object available in " + poolName);
            }
#endif
        }
#if UNITY_EDITOR
        ...
        {
            LogMgr.LogError("[ResourceManager]:Invalid pool name specified: " + poolName);
        }
#endif
        return result;
    }
    ...
    public void ReturnObjectToPool(GameObject go)
    {
        ...
        {
#if UNITY_EDITOR
            LogMgr.LogWarning("Specified object is not a pooled instance: " + go.name);
#endif
        }
        else
        {
            ...
            {
                LogMgr.LogWarning("No pool available with name: " + po.poolName);
            }
#endif
        }
    }
    ...
    public void ReturnTransformToPool(Transform t)
    {
        ...
        {
#if UNITY_EDITOR
            LogMgr.LogError("[ResourceManager] try to return a null transform to pool!");
#endif
            return;
        }
