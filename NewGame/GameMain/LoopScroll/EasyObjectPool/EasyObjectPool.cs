...
class Pool
{
    ...
    public Pool(string poolName, GameObject poolObjectPrefab, GameObject rootPoolObj, int initialCount, PoolInflationType type)
    {
        ...
        {
#if UNITY_EDITOR
            SEZSJ.LogMgr.LogError("[ObjPoolManager] null pool object prefab !");
#endif
            return;
        }
...
#if UNITY_EDITOR
            //LogMgr.Log(string.Format("Growing pool {0}: {1} populated", poolName, increaseSize));
#endif
            if (increaseSize > 0)
            {
    ...
    public void ReturnObjectToPool(PoolObject po)
    {
        if (poolName.Equals(po.poolName))
        {
            ...
            {
#if UNITY_EDITOR
                SEZSJ.LogMgr.LogWarning(po.gameObject.name + " is already in pool. Why are you trying to return it again? Check usage.");
#endif
            }
            else
            {
        ...
        {
            SEZSJ.LogMgr.LogError(string.Format("Trying to add object to incorrect pool {0} {1}", po.poolName, poolName));
        }
    }
}
