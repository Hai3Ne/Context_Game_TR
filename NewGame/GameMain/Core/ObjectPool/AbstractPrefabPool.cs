...
public abstract class AbstractPrefabPool<T> : IPrefabPool<T> where T : UnityEngine.Object
    {
        ...
        private float _fHoldTime = -1.0f;
        public float HoldTime
        {
            get { return _fHoldTime; }
            set { _fHoldTime = value; }
        }
         ...
         * Every item passes this method before it gets recycled
         */
        protected abstract void OnHandleDestroyPrefab(T prefabInstance);
        ...
        public void ClearnAllPoolObject()
        {
            if (UnrecycledPrefabCount <= 0) //所以对象池的对象都被回收了 才做销毁
            {
                int nCount = _availableInstances.Count;
                if (nCount > 0)
                {
                    for (int i = nCount - 1; i >=0; i--)
                    {
                        var prefab = _availableInstances[i];
                        if (prefab != null)
                        {
                            _availableInstances.RemoveAt(i);
                            OnHandleDestroyPrefab(prefab);
                        }
                    }
                    GC.Collect();
                }
        ...
        private T AllocatePoolItem()
	    {
            if (_objectToCopy == null)
                return null;
	        var instance = UnityEngine.Object.Instantiate(_objectToCopy, Vector3.zero, Quaternion.identity) as T;
