...
public class Item_SC : UIItem {
    public UISprite mSprIcon;
    public UILabel mLbCount;
    public UILabel mLbDay;
    public void InitData(uint item_id,uint num) {
        ...
        if (num < 10000) {
            this.mLbCount.text = string.Format("x{0}", num);
        } else {
            this.mLbCount.text = string.Format("{0}万", num / 10000);
        }
    }
    public void InitData_VIP(byte vip, int day) {
        ...
        this.mLbDay.text = string.Format("{0}日",day);
        this.mLbCount.text = string.Format("{0}会员", ConstValue.VIPName[vip]);
    }
    ...
    public override void OnNodeAsset(string name, Transform tf) {
        switch (name) {
            ...
            case "item_lb_count":
                ...
                break;
            case "item_lb_day":
                this.mLbDay = tf.GetComponent<UILabel>();
                break;
        }
    }
}
