// THAY THẾ NỘI DUNG TOÀN BỘ FILE UniWebView.cs
...
/// <summary>
/// PC Stub version of UniWebView - Opens URLs in system browser
/// </summary>
public class UniWebView : MonoBehaviour {
    
    // Events - same interface as mobile
    public delegate void LoadCompleteDelegate(UniWebView webView, bool success, string errorMessage);
    public event LoadCompleteDelegate OnLoadComplete;
    ...
    // Properties - same interface as mobile
    [SerializeField] private UniWebViewEdgeInsets _insets = new UniWebViewEdgeInsets(0,0,0,0);
    public UniWebViewEdgeInsets insets {
        get { return _insets; }
        set { _insets = value; Debug.Log($"[PC UniWebView] Insets set to: {value.top},{value.left},{value.bottom},{value.right}"); }
    }

    public string url;
    public bool loadOnStart = false;
    public bool autoShowWhenLoadComplete = false;
    public string currentUrl { get { return url; } }
    public bool backButtonEnable = true;
    public bool bouncesEnable = false;
    public bool zoomEnable = false;
    public string userAgent { get { return "PC UniWebView Stub"; } }
    public float alpha { get { return 1.0f; } set { /* PC doesn't support alpha */ } }
    public bool openLinksInExternalBrowser { get { return true; } set { /* Always true on PC */ } }
    public bool immersiveMode { get { return false; } set { /* PC doesn't support immersive */ } }
    public bool toolBarShow = false;

    // Static methods
    public static void SetUserAgent(string value) {
        Debug.Log($"[PC UniWebView] SetUserAgent: {value}");
    }

    public static void ResetUserAgent() {
        Debug.Log("[PC UniWebView] ResetUserAgent called");
    }

    public static void SetDoneButtonText(string text) {
        Debug.Log($"[PC UniWebView] SetDoneButtonText: {text}");
    }

    public static void SetWebContentsDebuggingEnabled(bool enabled) {
        Debug.Log($"[PC UniWebView] SetWebContentsDebuggingEnabled: {enabled}");
    }

    // Core methods
    public void Load() {
        string loadUrl = String.IsNullOrEmpty(url) ? "about:blank" : url.Trim();
        Debug.Log($"[PC UniWebView] Load called: {loadUrl}");
        
        if (OnLoadBegin != null) {
            OnLoadBegin(this, loadUrl);
        }

        if (!string.IsNullOrEmpty(loadUrl) && !loadUrl.Equals("about:blank")) {
            try {
                Application.OpenURL(loadUrl);
                Debug.Log($"[PC UniWebView] Opened URL in system browser: {loadUrl}");
                
                // Simulate successful load
                StartCoroutine(SimulateLoadComplete(true, null));
            } catch (Exception e) {
                Debug.LogError($"[PC UniWebView] Failed to open URL: {e.Message}");
                StartCoroutine(SimulateLoadComplete(false, e.Message));
            }
        } else {
            StartCoroutine(SimulateLoadComplete(true, null));
        }
    ...
    public void LoadHTMLString(string htmlString, string baseUrl) {
        Debug.Log($"[PC UniWebView] LoadHTMLString called - PC doesn't support HTML string loading");
        StartCoroutine(SimulateLoadComplete(false, "PC doesn't support HTML string loading"));
    }

    public void Reload() {
        Debug.Log("[PC UniWebView] Reload called");
        Load();
    }

    public void Stop() {
        Debug.Log("[PC UniWebView] Stop called");
    }

    public void Show(bool fade = false, UniWebViewTransitionEdge direction = UniWebViewTransitionEdge.None, float duration = 0.4f, Action finishAction = null) {
        Debug.Log("[PC UniWebView] Show called - URL should already be open in browser");
        finishAction?.Invoke();
    }

    public void Hide(bool fade = false, UniWebViewTransitionEdge direction = UniWebViewTransitionEdge.None, float duration = 0.4f, Action finishAction = null) {
        Debug.Log("[PC UniWebView] Hide called");
        finishAction?.Invoke();
    }

    public void EvaluatingJavaScript(string javaScript) {
        Debug.Log($"[PC UniWebView] EvaluatingJavaScript: {javaScript} - Not supported on PC");
        if (OnEvalJavaScriptFinished != null) {
            StartCoroutine(SimulateJSResult(""));
        }
    }

    public void AddJavaScript(string javaScript) {
        Debug.Log($"[PC UniWebView] AddJavaScript: {javaScript} - Not supported on PC");
    }

    public void CleanCache() {
        Debug.Log("[PC UniWebView] CleanCache called - PC doesn't have cache to clean");
    }

    public void CleanCookie(string key = null) {
        Debug.Log($"[PC UniWebView] CleanCookie: {key} - PC doesn't manage cookies");
    }

    [Obsolete("SetTransparentBackground is deprecated, please use SetBackgroundColor instead.")]
    public void SetTransparentBackground(bool transparent = true) {
        Debug.Log($"[PC UniWebView] SetTransparentBackground: {transparent}");
    }

    public void SetBackgroundColor(Color color) {
        Debug.Log($"[PC UniWebView] SetBackgroundColor: {color}");
    }

    public void ShowToolBar(bool animate) {
        Debug.Log($"[PC UniWebView] ShowToolBar: {animate}");
    }

    public void HideToolBar(bool animate) {
        Debug.Log($"[PC UniWebView] HideToolBar: {animate}");
    }

    public void SetShowSpinnerWhenLoading(bool show) {
        Debug.Log($"[PC UniWebView] SetShowSpinnerWhenLoading: {show}");
    }

    public void SetSpinnerLabelText(string text) {
        Debug.Log($"[PC UniWebView] SetSpinnerLabelText: {text}");
    }

    public void SetUseWideViewPort(bool use) {
        Debug.Log($"[PC UniWebView] SetUseWideViewPort: {use}");
    }

    public void LoadWithOverviewMode(bool overview) {
        Debug.Log($"[PC UniWebView] LoadWithOverviewMode: {overview}");
    }

    public bool CanGoBack() {
        Debug.Log("[PC UniWebView] CanGoBack: false");
        return false;
    }

    public bool CanGoForward() {
        Debug.Log("[PC UniWebView] CanGoForward: false");
        return false;
    }

    public void GoBack() {
        Debug.Log("[PC UniWebView] GoBack called - not supported");
    }

    public void GoForward() {
        Debug.Log("[PC UniWebView] GoForward called - not supported");
    }

    public void AddPermissionRequestTrustSite(string url) {
        Debug.Log($"[PC UniWebView] AddPermissionRequestTrustSite: {url}");
    }

    public void AddUrlScheme(string scheme) {
        Debug.Log($"[PC UniWebView] AddUrlScheme: {scheme}");
    }

    public void RemoveUrlScheme(string scheme) {
        Debug.Log($"[PC UniWebView] RemoveUrlScheme: {scheme}");
    }

    public void SetHeaderField(string key, string value) {
        Debug.Log($"[PC UniWebView] SetHeaderField: {key} = {value}");
    }

    public void SetVerticalScrollBarShow(bool show) {
        Debug.Log($"[PC UniWebView] SetVerticalScrollBarShow: {show}");
    }

    public void SetHorizontalScrollBarShow(bool show) {
        Debug.Log($"[PC UniWebView] SetHorizontalScrollBarShow: {show}");
    }

    // Lifecycle
    void Awake() {
        gameObject.name = gameObject.name + "_PC_" + System.Guid.NewGuid().ToString();
        Debug.Log($"[PC UniWebView] Awake: {gameObject.name}");
    }
    ...
    void OnDestroy() {
        Debug.Log($"[PC UniWebView] OnDestroy: {gameObject.name}");
    ...
    // Helper coroutines
    private IEnumerator SimulateLoadComplete(bool success, string errorMessage) {
        yield return new WaitForSeconds(0.5f); // Simulate loading time
        
        if (OnLoadComplete != null) {
            OnLoadComplete(this, success, errorMessage);
        }
        
        if (success && autoShowWhenLoadComplete) {
            Show();
        }
    }

    private IEnumerator SimulateJSResult(string result) {
        yield return new WaitForSeconds(0.1f);
        OnEvalJavaScriptFinished?.Invoke(this, result);
    }
}

// Static Plugin class for PC
public static class UniWebViewPlugin {
    public static void Init(string name, int top, int left, int bottom, int right) {
        Debug.Log($"[PC Plugin] Init: {name}");
    }
    
    public static void Load(string name, string url) {
        Debug.Log($"[PC Plugin] Load: {name} -> {url}");
    }
    
    public static string GetCurrentUrl(string name) { return ""; }
    ...
    public static string GetUserAgent(string name) { return "PC UniWebView"; }
    public static float GetAlpha(string name) { return 1.0f; }
    public static void SetAlpha(string name, float alpha) { }
    public static bool GetOpenLinksInExternalBrowser(string name) { return true; }
    ...
    public static int ScreenHeight() { return Screen.height; }
    public static int ScreenWidth() { return Screen.width; }
    public static int ScreenScale() { return 1; }
    public static int GetId(string name) { return 0; }
    public static IntPtr GetRenderEventFunc() { return IntPtr.Zero; }
    public static void InputEvent(string name, int x, int y, float deltaY, bool down, bool press, bool release, bool keyPress, short keyCode, string keyChars, int textureId) { }
}
