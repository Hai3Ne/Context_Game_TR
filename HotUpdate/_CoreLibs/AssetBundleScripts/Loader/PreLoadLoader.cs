...
namespace Kubility
{
    ...
    /// 同步加载器
    /// </summary>
    public class PreLoadLoader : AssetInterface, AssetRequestInterface, AssetLoaderDataInterface
    {
        ...
        public void DownLoad(string name)
        {
            ...
            if(KApplication.isPlaying)
                GameUtils.ResumeFromBreakPoint(DownloadPath, outputPath,Progresss);
        }
...
//                      DataRef.Retain(binObj);
        ...
        {
            List<ABData> abs = new List<ABData> ();
            ...
            int loadedNum = abs.Count;
			if (loadedNum > 0) {
				KAssetDispather.Instance.DispatchLoaderAll<Object> (LoaderType.PreLoad, abs.ToArray (), delegate(Object o) {
					loadedNum--;
					if (loadedNum <= 0)
						LoadMain (); 
				}, Progresss);
                    ...
                    KAssetBundleManger.TryGetBytes(LoaderData, (byte[] bys) =>
                    {
                        ...
                        if (bys != null && bys.Length > 0)
                        {
                            ...
                            if (node == null || node.Bundle == null)
                            {
								...
								KAssetDispather.Instance.LoadBundleFromMemeorySync(LoaderData.Abname, bys, delegate(AssetBundle assetBundle){
										if (assetBundle.LogNull())
											LoadAssetBundleAsset(assetBundle, node, bys.Length);
										else
											LogMgr.LogError(LoaderData.Abname+" is null");
								});
        ...
        private void LoadAssetBundleAsset(AssetBundleCreateRequest assetBundleReq, KAssetBundleRef abRef, long size)
        {
            if (assetBundleReq != null)
            {
                #if FULLLOG
                LogMgr.Log("开始同步加载。。。。开始加载对象assetbundle"+LoaderData.Abname);
                #endif
                if ((ABFileTag)LoaderData.FileType != ABFileTag.Scene)
                {
                    TryGetLoadObject(assetBundleReq.assetBundle, abRef, size);
                }
                else
                {
                    DataRef = KAssetBundleRef.Create(LoaderData, assetBundleReq.assetBundle, null, size);
                    DataRef.Lock();
                    Finish();
                }
            }
        ...
        private void TryGetLoadObject(AssetBundle assetBundle, KAssetBundleRef abRef, long size)
		{
			if (abRef == null || abRef.MainObject == null || abRef.Bundle == null) {
				KAssetDispather.Instance.LoadAssetSync (assetBundle, LoaderData.LoadName, delegate(Object asset) {
					if (asset.LogNull ()) {
						//设置对象引用
