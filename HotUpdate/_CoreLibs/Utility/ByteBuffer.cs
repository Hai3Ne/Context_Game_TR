...
public class ByteBuffer
{
	...
	public void write(byte[] right,int len )
	{
		if (right != null) {
			#if debug
			Debug.Log("ByteBuffer 原来的数据大小为  "+left.DataCount.ToString()+"  新数据大小为 "+right.Length.ToString());
			#endif
			int nextsize = len + this.Position;
			if (nextsize > this.buffer.Length) {
				this.IncreaseCapacity (nextsize);
			}

			Array.Copy (right, 0, this.buffer, this.Position,len);
			this.Position +=len;
		}
	...
	public virtual byte[] Read (int begin, int len)
	{
		...
		int i = 0;
		for (i = 0; i < len; ++i) {
			bys [i] =buffer [begin +i];

		}
		//
		//						if (DataCount - len - 1 > 0) {
	...
	public static explicit  operator Bool8 (ByteBuffer left)
	{
		...
		Debug.LogError ("Read from ByteBuffer Error");
