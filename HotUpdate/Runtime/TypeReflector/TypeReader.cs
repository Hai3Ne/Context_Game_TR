...
public class BinStringReader : ITypeReader
{
    public object Read(BinaryReader bin, int count)
    {
		if (count == 0) {
			...
			string ch = "\0";
			do{
				ch = System.Text.Encoding.Unicode.GetString(bin.ReadBytes(2));
                sb.Append(ch);
			}while(ch != "\0" && bin.BaseStream.Position < bin.BaseStream.Length);
			return sb.ToString();
		}
...
public class TypeReaderMapping
{
	...
	public static ITypeReader GetTypeReader<T>()
	{
		int HASH = Utility.GetHash(typeof(T).ToString());
		return GetReaderFromHash (HASH);
	}
