...
public class TypeFiledPack
{
    ...
    public ushort        Count;
    public bool         VarType;
    public TypeFiledPack(short idx, ushort count, bool varType, FieldInfo fi)
    {
        Idx = idx;
        Count = count;
        Field = fi;
...
public class TypeSortedField
{
    ...
    public bool AddField(FieldInfo fi, short idx, ushort count,bool varType)
    {
...
public class TypeMapping
{
    ...
    static TypeInfo GetFieldTypeInfo(FieldInfo fi)
    {
        ...
        if (attri != null && attri.Length > 0)
        {
            ...
            if (arr == null)
            {
                Debug.Log("未知的特性:" + attri[0].ToString());
            }
            return arr;
        }
        return null;
	...
	static bool ChkTypeParseAttr(Type t)
	{
		object[] attrs = t.GetCustomAttributes (false);
		return attrs != null && attrs.Length > 0 && ((attrs [0] as CustomTypeParse) != null);
	}
    ...
    public static TypeData RegisterClass(Type t)
    {
		...
		td.parseFlag = ChkTypeParseAttr (t) ? 1 : 0;
        int idx = 0;
        for (int n = 0; n < sortFields.Count; ++n)
        {
            for (int i = 0; i < sortFields[n].Fields.Count; ++i)
            {
                ...
                else if (fi.FieldType.IsArray)
                {
                    ...
                    if (orgType == null)
                    {
                        Debug.Log("未知的数组类型:" + strType);
                        return null;
                    }
                    if(field.Count < 1 && field.VarType == false)
                    {
                        Debug.Log("数组的特性TypeInfo必须大于0，或者是变长的数组");
                        return null;
                    }
                ...
                else if (fi.FieldType.IsClass /*|| fi.FieldType.IsValueType*/)
                {
                    ...
                    if (curData == null) return null;
                    curData.Handle = fi;
					if (td.parseFlag == 0 && curData.parseFlag != 0)
						td.parseFlag = 2;
                }
                else
                {
                    Debug.Log("未知的类型:" + fi.FieldType.ToString());
                    return null;
                }
