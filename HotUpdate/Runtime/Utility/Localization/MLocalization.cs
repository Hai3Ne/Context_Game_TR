...
{
    static MLocalization mInstance;
    ...
    static public MLocalization Instance
    {
        get
        {
            if (mInstance == null)
            {
                mInstance = Object.FindObjectOfType(typeof(MLocalization)) as MLocalization;

                if (mInstance == null)
                {
                    GameObject go = new GameObject("_Localization");
                    DontDestroyOnLoad(go);
                    mInstance = go.AddComponent<MLocalization>();
                }
            }
            return mInstance;
        }
    ...
    public string currentLanguage
    {
        ...
        set
        {
            if (mLanguage != value)
            {
                ...
                {
                    //Kubility.KAssetBundleManger.Instance.ResourceLoad<TextAsset>("LocalizationText/" + value, delegate (SmallAbStruct txt)
                    //{
                    //    if (!txt.IsNull())
                    //    {
                    //        TextAsset info = txt.MainObject as TextAsset;
                    //        Load(info);
                    //        KAssetBundleManger.Instance.ResourceLoad<Font>("Font/font", (data) =>
                    //         {

                    //             if (!data.IsNull())
                    //             {
                    //                 Font font = data.MainObject as Font;
                    //                 font.RequestCharactersInTexture(info.text, 20);
                    ...
                    //},LoaderType.YieldLoad);


                    TextAsset textAsset = ResManager.LoadAsset<TextAsset>(GameEnum.Fish_3D,"LocalizationText/" + value);
                    if (textAsset != null)
                    {
                        Load(textAsset);
                        Font data = ResManager.LoadAsset<Font>(GameEnum.Fish_3D,"Font/font");

                        if (data != null)
                        {
                            data.RequestCharactersInTexture(textAsset.text, 20);
                        }
                    }
                    else
                    {
    ...
    void Awake()
    {
        if (debugLanguage != SystemLanguage.Unknown)
            PlayerPrefs.DeleteKey("Language");
        if (mInstance == null)
        {
            mInstance = this;
            ...
            else
            {
                ...
                {
                    currentLanguage = LanguagesPath();
                }

                //LogMgr.Log("currentLanguage=" + currentLanguage);
                //LogMgr.Log("debugLanguage=" + debugLanguage);

            }
            //LogMgr.Log(PlayerPrefs.GetString("Language"));
            //
            //			if (Application.platform == RuntimePlatform.IPhonePlayer) {
            //				PlatformIns.Instance.Save ("Language", MLocalization.Instance.currentLanguage);
            //				PlatformIns.Instance.Save ("FPath", Application.streamingAssetsPath + "/LocalizationText/" + MLocalization.Instance.currentLanguage + ".txt");
            //			}
    ...
    {
        if (debugLanguage == SystemLanguage.Unknown)
            debugLanguage = SystemLanguage.English;
        //            debugLanguage = Application.systemLanguage;
//        debugLanguage = SystemLanguage.Chinese;
