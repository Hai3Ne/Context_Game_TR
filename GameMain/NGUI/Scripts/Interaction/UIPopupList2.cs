...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/Popup List2")]
public class UIPopupList2 : UIWidgetContainer
{
	/// <summary>
	/// Current popup list. Only available during the OnSelectionChange event callback.
	/// </summary>

	static public UIPopupList2 current;
	...
	[System.Obsolete("Use EventDelegate.Add(popup.onChange, YourCallback) instead, and UIPopupList2.current.value to determine the state")]
	...
	Vector3 GetHighlightPosition ()
	{
		...
		return new Vector3(0, mHighlightedLabel.cachedTransform.localPosition.y);
	}
	...
	public void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
            ...
            UIWidget widget = myTrans.GetComponent<UIWidget>();
            ...
            t.localPosition = myTrans.localPosition;
            ...
            mBackground.width = widget.width;

            mBgBorder = widget.height * 0.5f;
            mBackground.cachedTransform.localPosition = new Vector3(0f, -mBgBorder, 0f);

			// Add a sprite used for the selection
			mHighlight = NGUITools.AddSprite(mChild, atlas, highlightSprite);
            mHighlight.pivot = UIWidget.Pivot.Center;
			mHighlight.color = highlightColor;

			UISpriteData hlsp = mHighlight.GetAtlasSprite();
			if (hlsp == null) return;

            float item_height = padding.y;
            float y = -item_height * 1.5f;
            List<UILabel> labels = new List<UILabel>();

            mBackground.height = (int)(item_height * items.Count + item_height);
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				lbl.color = textColor;
                lbl.cachedTransform.localPosition = new Vector3(0, y, -1f);
                ...
                if (bc != null) {
                    bc.center = new Vector3(0, 0);
                    bc.size = new Vector3(mBackground.width, item_height);
                } else {
                    BoxCollider2D b2d = lbl.GetComponent<BoxCollider2D>();
#if UNITY_4_3 || UNITY_4_5 || UNITY_4_6
					b2d.center = new Vector3(0, 0);
#else
                    b2d.offset = new Vector3(0, 0);
#endif
                    b2d.size = new Vector3(mBackground.width, item_height);
                }
			}

            mHighlight.width = Mathf.RoundToInt(mBackground.width - padding.x * 2);
			mHighlight.height = Mathf.RoundToInt(item_height);
            ...
            if (placeAbove) {
                ...
                p.y += mBackground.height + widget.height;
                t.localPosition = p;
            }
		}
		else OnSelect(false);
