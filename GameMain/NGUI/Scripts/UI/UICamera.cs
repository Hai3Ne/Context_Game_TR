...
public class UICamera : MonoBehaviour
{
	...
	public delegate bool GameObjFun(GameObject go);
	public static  GameObjFun ChkGameObjCanClick = delegate(GameObject go) {
		return true;
	};
			...
			else if (cam.eventType == EventType.UI_3D)
			{
				...
				if (hits.Length > 1)
				{
					for (int b = 0; b < hits.Length; ++b)
					{
						...
						if (mHit.depth != int.MaxValue && (ChkGameObjCanClick == null || ChkGameObjCanClick(go)))
						{
							mHit.hit = hits[b];
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
		...
		if (NGUITools.GetActive(go))
		{
			...
			if (ChkGameObjCanClick == null || ChkGameObjCanClick(go))
