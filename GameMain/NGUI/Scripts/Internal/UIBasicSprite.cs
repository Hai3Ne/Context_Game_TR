...
#region Fill Functions
	...
	protected void Fill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Rect outer, Rect inner)
	{
		...
		switch (type)
		{
            ...
            case Type.Circle:
            CircleFill(verts, uvs, cols);
            break;
		}
	}
    void CircleFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols) {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color32 color = drawingColor;

        Vector4 rv = new Vector4((v.x + v.z) * 0.5f, (v.y + v.w) * 0.5f, (v.z - v.x) * 0.5f, (v.w - v.y) * 0.5f);
        Vector4 ru = new Vector4((u.x + u.z) * 0.5f, (u.y + u.w) * 0.5f, (u.z - u.x) * 0.5f, (u.w - u.y) * 0.5f);

        int segments = 36;//·Ö¸î36¿é
        float angleByStep = (Mathf.PI * 2f) / segments;
        float currentAngle = 0f;
        Vector2 pre_v = Vector2.zero;
        for (int i = 0; i < segments >> 1; i++) {
            float c1 = Mathf.Cos(currentAngle);
            float s1 = Mathf.Sin(currentAngle);
            float c2 = Mathf.Cos(currentAngle + angleByStep);
            float s2 = Mathf.Sin(currentAngle + angleByStep);
            float c3 = Mathf.Cos(currentAngle + 2 * angleByStep);
            float s3 = Mathf.Sin(currentAngle + 2 * angleByStep);

            Vector2 p1 = new Vector2(rv.x, rv.y);
            Vector2 p2 = new Vector2(rv.x, rv.y) + new Vector2(rv.z * c1, rv.w * s1);
            Vector2 p3 = new Vector2(rv.x, rv.y) + new Vector2(rv.z * c2, rv.w * s2);
            Vector2 p4 = new Vector2(rv.x, rv.y) + new Vector2(rv.z * c3, rv.w * s3);
            Vector2 u1 = new Vector2(ru.x, ru.y);
            Vector2 u2 = new Vector2(ru.x, ru.y) + new Vector2(ru.z * c1, ru.w * s1);
            Vector2 u3 = new Vector2(ru.x, ru.y) + new Vector2(ru.z * c2, ru.w * s2);
            Vector2 u4 = new Vector2(ru.x, ru.y) + new Vector2(ru.z * c3, ru.w * s3);
            verts.Add(p1);
            verts.Add(p2);
            verts.Add(p3);
            verts.Add(p4);
            ...
            currentAngle += angleByStep + angleByStep;
        }
    }
